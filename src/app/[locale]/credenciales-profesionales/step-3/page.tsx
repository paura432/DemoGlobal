...
export default function Step3() {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/locales/credenciales-profesionales/step-3/${locale}.json`, {
        cache: 'no-store',
      });
        ...
        <div className="w-full flex justify-center pt-1 pb-6 relative">
          ...
          {PASOS.map((_, i) => (
              <div key={i} className="flex items-center">
                ...
                {i < PASOS.length - 1 && (
                  <div className="w-8 h-[2px] bg-[#414B61] mx-1 sm:mx-2" />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="w-full max-w-[1100px] px-4 sm:px-10 mt-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start min-h-[300px]">
            <div className="min-h-[280px] mt-2">
              <h2
                className="text-2xl font-semibold text-[#414B61] mb-4"
              ...
              <ul className="mb-5 space-y-3">
                {block.instrucciones.map((line, i) => (
                  <li
                    key={i}
                    className="flex gap-2 items-start text-sm text-[#414B61] leading-relaxed"
                  >
                    <TickSmall />
                    <span
            ...
            <div className="flex justify-center lg:justify-end">
              {block.id !== 'qr' && block.video && (
                block.id === 'import' ? (
                  <button
                    ...
                    title={UI[locale].gotoStep2Part2}
                  >
              ...
              {block.id === 'qr' && (
                <div className="rounded-2xl shadow-lg bg-white border border-[#414B61] p-5">
                  <div className="w-[180px] md:w-[220px] lg:w-[260px] mx-auto grid place-items-center">
                    ...
                    {issuedWrapQr ? (
                      <div className="flex flex-col items-center">
                        ...
                        />
                        {issuing && (
                          <span className="mt-1 text-xs text-[#414B61]">
                            {UI[locale].generating}
                          </span>
                        )}
                      </div>
                    ) : (
                      <span className="text-[#414B61] text-sm">
