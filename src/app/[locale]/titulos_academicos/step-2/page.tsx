...
export default function Step2() {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/locales/titulos_academicos/step-2/${locale}.json`, {
        cache: 'no-store'
      });
  ...
  const goTo = (i: number) => {
    const step = PASOS[i];
    if (step) router.push(`/${locale}/titulos_academicos/${step}`);
  };
        ...
        <div className="w-full max-w-[1020px] px-4 sm:px-10 pt-5">
          ...
          <div className="relative flex items-center mb-16">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
              ...
              {idx === 0 && (
                <button
                  onClick={() => router.push(`/${locale}/titulos_academicos/step-3`)}
