...
export default function Step4() {
  ...
  useEffect(() => {
    (async () => {
      const [r4, r3] = await Promise.all([
        fetch(`/locales/titulos_academicos/step-4/${locale}.json`, { cache: 'no-store' }),
        fetch(`/locales/titulos_academicos/step-3/${locale}.json`, { cache: 'no-store' }),
      ]);
  ...
  const goBack = () => router.push(`/${locale}/titulos_academicos/step-3`);
