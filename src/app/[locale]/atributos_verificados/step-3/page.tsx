...
type Block = {
  ...
  instruccionesByRole?: Partial<Record<Rol, string[]>>;
  video?: string;
  videoByRole?: Partial<Record<Rol, string>>;
  poster?: string;
  posterByRole?: Partial<Record<Rol, string>>;
  apps?: AppItem[];
  boton?: string;
  credencialImagen?: string;
  credencialImagenByRole?: Partial<Record<Rol, string>>;
  modalBullets?: string[];
...
export default function Step3() {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/locales/atributos_verificados/step-3/${locale}.json`, { cache: 'no-store' });
  ...
  useEffect(() => {
    const saved = (typeof window !== 'undefined' ? (localStorage.getItem('tt_role') as Rol) : null) || 'medico';
    setRol(saved);
  }, []);

  const allBlocks = t?.bloques ?? [];
  const blocks = useMemo(() => allBlocks.filter((b) => b.id !== 'use'), [allBlocks]);
  const block = blocks[Math.max(0, Math.min(subIdx, Math.max(0, blocks.length - 1)))];

  const instrucciones: string[] = useMemo(() => {
    const base = block?.instrucciones ?? [];
    const extra = block?.instruccionesByRole?.medico ?? [];
    return [...base, ...extra];
  }, [block]);

  const videoSrc = useMemo(() => {
    if (!block) return '';
    return block.videoByRole?.medico ?? block.video ?? '';
  }, [block]);

  const posterSrc = useMemo(() => {
    if (!block) return undefined;
    return block.posterByRole?.medico ?? block.poster;
  }, [block]);
      ...
      <div className="flex-1 overflow-y-auto flex flex-col items-center">
          ...
          <div className="flex gap-2 items-center">
            {PASOS.map((_, i) => (
              <div key={i} className="flex items-center">
                <div className={`w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center border ${i === 2 ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-400 border-gray-300'}`}>{i + 1}</div>
            ...
            <div className="min-h-[280px] mt-2">
              ...
              {block.id === 'install' && (block.apps ?? []).length > 0 && (
                <div className="flex gap-8 items-center">
                  {(block.apps ?? []).map((app) => (
                    <button key={app.id} type="button" onClick={() => openAppModal(app)} className="group flex flex-col items-center focus:outline-none" title={app.nombre} aria-label={app.nombre}>
              ...
              {block.id === 'qr' && issueError && <p className="mt-2 text-sm text-red-600">{UI[locale].issueErrorPrefix}{issueError}</p>}
            </div>

            <div className="flex justify-center lg:justify-end">
              {block.id !== 'qr' && videoSrc && (
                block.id === 'import' ? (
                  <button type="button" onClick={goStep2Part2} className="cursor-pointer" title={UI[locale].gotoStep2Part2}>
                    <MediaCard mediaSrc={videoSrc} poster={posterSrc} />
                  </button>
                ) : (
                  <MediaCard mediaSrc={videoSrc} poster={posterSrc} />
                )
        ...
        <div className="w-full max-w-[1020px] px-4 sm:px-10 pt-5">
          ...
          <div className="flex justify-between items-center mb-16">
            <button onClick={goBack} className="border border-blue-600 text-blue-600 px-8 py-2 rounded-full text-sm font-medium transition hover:bg-blue-50">{UI[locale].back}</button>
            <div className="flex-1 flex justify-center">
              <button onClick={goNext} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2 rounded-full text-sm font-medium transition">
